...
hyp = {'giou': 1.582,  # giou loss gain
       ...
       'lr0': 0.0025,  # initial learning rate (SGD=1E-3, Adam=9E-5)
...
def train():
    ...
    def adjust_learning_rate(optimizer, gamma, epoch, iteration, epoch_size):
        ...
        if epoch < 6:
            ...
            if epoch > opt.epochs * 0.7:
                step_index = 5.99
            if epoch <= 0.2 * opt.epochs:
                step_index = 5
                gamma = 1
            lr = hyp['lr0'] * gamma *  (6 - step_index)
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        if save:
            ...
            torch.save(chkpt, wdir + 'epoch%g.pt' % epoch)
            if best_fitness == fitness:
                chkpt = {'model': model.module.state_dict() if type(model) is nn.parallel.DistributedDataParallel else model.state_dict()}
                torch.save(chkpt, best)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data', type=str, default='data/data.data', help='*.data file path')
    ...
    parser.add_argument('--nosave', action='store_true', help='only save final checkpoint', default=False)
    ...
    parser.add_argument('--cache-images', action='store_true', help='cache images for faster training', default=True)
    parser.add_argument('--weights', type=str, default='', help='ineatial weights')  # i.e. weights/darknet.53.conv.74
    ...
    parser.add_argument('--adam', action='store_true', help='use adam optimizer', default=True)
    parser.add_argument('--var', type=float, help='debug variable')
    parser.add_argument('--sparsity-regularization', '-sr', dest='sr', action='store_true',
                        help='train with channel sparsity regularization', default=False)
